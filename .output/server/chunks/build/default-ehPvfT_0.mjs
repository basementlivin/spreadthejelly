import { b as usePrismic, g as useSeoMeta, e as useAsyncData, a as __nuxt_component_0$1, _ as _export_sfc, h as PrismicLink } from './server.mjs';
import { useSSRContext, defineComponent, computed, mergeProps, ref, withCtx, createVNode, toDisplayString, watch, unref, resolveDynamicComponent, createTextVNode } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrRenderSlot, ssrRenderAttr, ssrInterpolate, ssrRenderList, ssrRenderClass, ssrRenderVNode } from 'vue/server-renderer';
import Logo from './logo--header-BztN-_k6.mjs';
import MenuHamburger from './menu--hamburger-WPKfLGaA.mjs';
import MenuX from './menu--x-DEsoN-py.mjs';
import { _ as __nuxt_component_1$1 } from './nuxt-img-DVe67KAQ.mjs';
import { isFilled } from '@prismicio/client';
import Cookies from 'universal-cookie';
import FooterMask from './mask--footer-CBWsNEnc.mjs';
import FooterLogo from './logo--footer-D7TvqE5C.mjs';
import FacebookIcon from './social--facebook-DJ4hApme.mjs';
import InstagramIcon from './social--instagram-B9m0dnf4.mjs';
import TikTokIcon from './social--tiktok-B9tNR2PA.mjs';
import YouTubeIcon from './social--youtube-BgYEpkwn.mjs';
...
_round$1 = function _round(value) {
  ...
  return ScrollTrigger$2.maxScroll(scroller || _win$2);
},
  ...
  var parent = el.parentNode || _docEl$2,
  ...
  if (change > 0) {
    ...
    ratio = b2.height / (_win$2.innerHeight + b2.height);
  ...
  var wrapper = _doc$2.querySelector(".ScrollSmoother-wrapper"); // some frameworks load multiple times, so one already exists, just use that to avoid duplicates
...
var ScrollSmoother = /*#__PURE__*/function () {
  function ScrollSmoother(vars) {
    ...
    ScrollSmoother.register(gsap$2) || console.warn("Please gsap.registerPlugin(ScrollSmoother)");
    vars = this.vars = vars || {};
    ...
    var _vars = vars,
        ...
        scrollFunc = ScrollTrigger$2.getScrollFunc(_win$2),
      ...
      if (arguments.length) {
        ...
        ScrollTrigger$2.isRefreshing ? mainST.update() : scrollFunc(value / speed); // during a refresh, we revert all scrollers to 0 and then put them back. We shouldn't force the window to that value too during the refresh.
      ...
      if (!ScrollTrigger$2.isRefreshing) {
      ...
      if (effects) {
        ...
        ScrollTrigger$2.getAll().forEach(function (st) {
      ...
      var getSpeed = effectValueGetter("speed", speed, index, el),
          ...
          startY = gsap$2.getProperty(el, "y"),
          cache = el._gsap,
          ratio,
          st,
          autoSpeed,
        ...
        if (autoSpeed) {
          ...
          var auto = _autoDistance(el, _clamp$1(0, 1, -self.start / (self.end - self.start)));
          ...
          onUpdate: function onUpdate(self) {
            ...
            if (self.offset) {
              ...
              if (i) {
                ...
                y = startY + extraY + change * ((gsap$2.utils.clamp(self.start, self.end, scrollY) - self.start - extraY) / (end - self.start) - progressOffset);
              }

              markers.length && !autoSpeed && markers.forEach(function (setter) {
                return setter(y - extraY);
              });
              y = _round$1(y + yOffset);
    ...
    this.scrollTo = function (target, smooth, position) {
      var p = gsap$2.utils.clamp(0, _maxScroll$1(), isNaN(target) ? _this.offset(target, position, !!smooth && !paused) : +target);
      !smooth ? scrollTop(p) : paused ? gsap$2.to(_this, {
    ...
    this.offset = function (target, position, ignoreSpeed) {
      ...
      if (effects) {
        startupPhase ? ScrollTrigger$2.refresh() : adjustParallaxPosition([st], true); // all the effects need to go through the initial full refresh() so that all the pins and ratios and offsets are set up. That's why we do a full refresh() if it's during the startupPhase.
      }
    ...
    function refreshHeight() {
      ...
      _body$2.style.height = _win$2.innerHeight + (height - _win$2.innerHeight) / speed + "px";
      return height - _win$2.innerHeight;
    }

    this.content = function (element) {
      if (arguments.length) {
        var newContent = _toArray$1(element || "#smooth-content")[0] || console.warn("ScrollSmoother needs a valid content element.") || _body$2.children[0];
        ...
        if (newContent !== content) {
          ...
          smoothDuration || gsap$2.set(content, {
    ...
    this.effects = function (targets, config) {
      ...
      config.refresh !== false && ScrollTrigger$2.refresh(); // certain effects require a refresh to work properly
    ...
    ScrollTrigger$2.scrollerProxy(wrapper, {
      ...
      scrollHeight: function scrollHeight() {
        return refreshHeight() && _body$2.scrollHeight;
      },
      ...
      getBoundingClientRect: function getBoundingClientRect() {
        return {
          top: 0,
          left: 0,
          width: _win$2.innerWidth,
          height: _win$2.innerHeight
        };
      }
    });
    ...
    var existingScrollTriggers = ScrollTrigger$2.getAll().filter(function (st) {
      return st.scroller === _win$2 || st.scroller === wrapper;
    });
    ...
    mainST = ScrollTrigger$2.create({
      animation: gsap$2.fromTo(scroll, {
        y: function y() {
          allowUpdates = 0;
          return 0;
        }
      ...
      onRefreshInit: function onRefreshInit(self) {
        ...
        if (effects) {
          var _pins = ScrollTrigger$2.getAll().filter(function (st) {
            return !!st.pin;
          });
        ...
        if (smoothDuration) {
          ...
          ScrollTrigger$2.isTouch === 1 && (wrapper.style.position = "fixed");
        }
      },
      onRefresh: function onRefresh(self) {
        ...
        if (!startupPhase) {
          ...
          self.animation.progress(gsap$2.utils.clamp(0, 1, recordedRefreshScroll / speed / -self.end));
        }
      ...
      if (arguments.length) {
        if (!!paused !== value) {
          if (value) {
            ...
            paused.nested = _inputObserver$1(_docEl$2, "wheel,touch,scroll", true, allowNestedScroll !== false); // allow nested scrolling, like modals
          } else {
            // resume
            paused.nested.kill();
            paused.kill();
    ...
    this.kill = this.revert = function () {
      ...
      _body$2.style.removeProperty("height");

      _win$2.removeEventListener("focusin", _onFocusIn);
    };
    ...
    ScrollTrigger$2.config(vars); // in case user passes in ignoreMobileResize for example

    "overscrollBehavior" in _win$2.getComputedStyle(_body$2) && gsap$2.set([_body$2, _docEl$2], {
      overscrollBehavior: "none"
    });
    "scrollBehavior" in _win$2.getComputedStyle(_body$2) && gsap$2.set([_body$2, _docEl$2], {
      scrollBehavior: "auto"
    }); // if the user hits the tab key (or whatever) to shift focus to an element that's off-screen, center that element.

    _win$2.addEventListener("focusin", _onFocusIn);
  ...
  ScrollSmoother.register = function register(core) {
    {
      ...
      if (gsap$2) {
        ...
        gsap$2.core.globals("ScrollSmoother", ScrollSmoother); // must register the global manually because in Internet Explorer, functions (classes) don't have a "name" property.
      }
    }
...
_getGSAP$2() && gsap$2.registerPlugin(ScrollSmoother);
...
const _sfc_main$4 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props, { emit: __emit }) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "search-component" }, _attrs))} data-v-8d7131ec><input${ssrRenderAttr("value", searchQuery.value)} type="text" placeholder="Search articles..." data-v-8d7131ec>`);
      if (loading.value) {
        _push(`<div data-v-8d7131ec> Searching... </div>`);
      } else {
        _push(`<!---->`);
      }
      if (error.value) {
        _push(`<div data-v-8d7131ec>${ssrInterpolate(error.value)}</div>`);
      } else {
        _push(`<!---->`);
      }
      if (results.value.length) {
        _push(`<ul class="search-results" data-v-8d7131ec><!--[-->`);
        ssrRenderList(results.value, (result) => {
          _push(`<li class="result" data-v-8d7131ec>`);
          _push(ssrRenderComponent(_component_NuxtLink, {
            to: `/${result.type === "ask_jelly_article" ? "ask-jelly" : "articles"}/${result.uid}`,
            onClick: ($event) => emit("close-search-modal")
          }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                ...
                })}" data-v-8d7131ec${_scopeId}>`);
                _push2(ssrRenderComponent(_component_NuxtImg, {
                  src: result.data.featured_image.url,
                  alt: result.data.featured_image.alt
                }, null, _parent2, _scopeId));
                _push2(`</div><span class="title" data-v-8d7131ec${_scopeId}>${ssrInterpolate(result.data.title)}</span>`);
              } else {
                return [
                  createVNode("div", {
                    class: {
                  ...
                  createVNode("span", { class: "title" }, toDisplayString(result.data.title), 1)
                ];
              }
            }),
            _: 2
...
const Search = /* @__PURE__ */ _export_sfc(_sfc_main$4, [["__scopeId", "data-v-8d7131ec"]]);
const _sfc_main$3 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    const trapFocusModal = (event) => {
      if (!isSearchModalOpen.value || !searchModalRef.value)
        return;
    ...
    const trapFocusMenu = (event) => {
      if (!isMenuOpen.value || !navMenuRef.value || !menuToggleRef.value)
        return;
    ...
    watch(isSearchModalOpen, (newVal) => {
      if (newVal) {
        (void 0).addEventListener("keydown", trapFocusModal);
      } else {
        (void 0).removeEventListener("keydown", trapFocusModal);
      }
    });
    watch(isMenuOpen, (newVal) => {
      if (newVal) {
        (void 0).addEventListener("keydown", trapFocusMenu);
      } else {
        (void 0).removeEventListener("keydown", trapFocusMenu);
      }
    });
    return (_ctx, _push, _parent, _attrs) => {
      var _a;
      ...
      _push(`<header${ssrRenderAttrs(mergeProps({ class: "header" }, _attrs))} data-v-d0f547bc><div class="header__inner" data-v-d0f547bc>`);
      _push(ssrRenderComponent(_component_NuxtLink, {
        to: "/",
        class: "logo",
        onClick: closeMenu
      ...
      _push(`<button class="menu-toggle" data-v-d0f547bc>`);
      ssrRenderVNode(_push, createVNode(resolveDynamicComponent(isMenuOpen.value ? unref(MenuX) : unref(MenuHamburger)), null, null), _parent);
      _push(`</button></div><nav class="${ssrRenderClass([{ "menu-open": isMenuOpen.value }, "slide-in-menu"])}" data-v-d0f547bc><div class="slide-in-menu__inner" data-v-d0f547bc><ul data-v-d0f547bc><!--[-->`);
      ssrRenderList((_a = unref(navigation)) == null ? void 0 : _a.data.links, (item) => {
        _push(`<li class="menu-item h4" data-v-d0f547bc>`);
        _push(ssrRenderComponent(_component_PrismicLink, {
          ...
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(`${ssrInterpolate(_ctx.$prismic.asText(item.label))}`);
            } else {
              return [
      ...
      _push(`<!--]--><li class="menu-item h4" data-v-d0f547bc><a href="#" aria-label="Open search dialog" tabindex="0" data-v-d0f547bc> Search </a></li></ul></div></nav>`);
      if (isSearchModalOpen.value) {
        _push(`<div class="search-modal" role="dialog" aria-modal="true" aria-labelledby="search-dialog-title" data-v-d0f547bc><div class="search-modal__content" data-v-d0f547bc><span id="search-dialog-title" class="sr-only" data-v-d0f547bc> Search Dialog </span><button class="close-button" aria-label="Close search dialog" data-v-d0f547bc>`);
        _push(ssrRenderComponent(unref(MenuX), null, null, _parent));
        _push(`</button>`);
        _push(ssrRenderComponent(Search, { onCloseSearchModal: toggleSearchModal }, null, _parent));
        _push(`</div></div>`);
      } else {
        _push(`<!---->`);
      }
...
const __nuxt_component_0 = /* @__PURE__ */ _export_sfc(_sfc_main$3, [["__scopeId", "data-v-d0f547bc"]]);
const useNewsletterPopupForm = () => {
  const prismic = usePrismic();
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      if (isModalVisible.value) {
        _push(`<div${ssrRenderAttrs(mergeProps({ class: "modal-overlay" }, _attrs))} data-v-b9ed10d4><div class="${ssrRenderClass({
          "modal-content": true,
          "newsletter-popup": true
        })}" data-v-b9ed10d4><button class="close-button sr-only" aria-label="Close signup modal" data-v-b9ed10d4> X </button>`);
        if (isFormSubmitted.value) {
          _push(`<div class="success-message" data-v-b9ed10d4>`);
          if (unref(isFilled).keyText((_a = unref(newsletter)) == null ? void 0 : _a.data.success_headline)) {
            _push(`<span class="headline" data-v-b9ed10d4>${ssrInterpolate(((_b = unref(newsletter)) == null ? void 0 : _b.data.success_headline) || "Got it, thanks!")}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(isFilled).keyText((_c = unref(newsletter)) == null ? void 0 : _c.data.success_subheadline)) {
            _push(`<span class="subheadline" data-v-b9ed10d4>${ssrInterpolate((_d = unref(newsletter)) == null ? void 0 : _d.data.success_subheadline)}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div>`);
        } else {
          _push(`<form name="newsletter" method="POST" data-netlify="true" class="signup-form" data-v-b9ed10d4><input type="hidden" name="newletter-signup-popup" value="newsletter" autocomplete="email" data-v-b9ed10d4><div class="signup-form__intro" data-v-b9ed10d4>`);
          if (unref(isFilled).keyText((_e = unref(newsletter)) == null ? void 0 : _e.data.headline)) {
            _push(`<span class="headline" data-v-b9ed10d4>${ssrInterpolate(((_f = unref(newsletter)) == null ? void 0 : _f.data.headline) || "Get Jelly")}</span>`);
          } else {
            _push(`<!---->`);
          }
          if (unref(isFilled).keyText((_g = unref(newsletter)) == null ? void 0 : _g.data.subheadline)) {
            _push(`<span class="subheadline" data-v-b9ed10d4>${ssrInterpolate(((_h = unref(newsletter)) == null ? void 0 : _h.data.subheadline) || "Sign up for the newsletter")}</span>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div><div class="signup-form__content" data-v-b9ed10d4><label for="email" class="sr-only" data-v-b9ed10d4> Email Address </label><input id="email" type="email" name="email" placeholder="Your email" required data-v-b9ed10d4>`);
          if (unref(isFilled).keyText((_i = unref(newsletter)) == null ? void 0 : _i.data.button_text)) {
            _push(`<button type="submit" class="link" data-v-b9ed10d4>${ssrInterpolate(((_j = unref(newsletter)) == null ? void 0 : _j.data.button_text) || "Join the club!")}</button>`);
          } else {
            _push(`<!---->`);
          }
          _push(`</div></form>`);
...
_sfc_main$2.setup = (props, ctx) => {
  const ssrContext = useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("components/NewsletterSignup.vue");
  return _sfc_setup$2 ? _sfc_setup$2(props, ctx) : void 0;
};
const __nuxt_component_1 = /* @__PURE__ */ _export_sfc(_sfc_main$2, [["__scopeId", "data-v-b9ed10d4"]]);
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_PrismicLink = PrismicLink;
      _push(`<!--[--><div class="footer__mask" data-v-31e86aa7>`);
      _push(ssrRenderComponent(unref(FooterMask), { class: "mask" }, null, _parent));
      _push(`</div><footer class="footer" data-scroll-section data-v-31e86aa7><div class="footer__logo" data-v-31e86aa7>`);
      _push(ssrRenderComponent(unref(FooterLogo), { class: "logo" }, null, _parent));
      _push(`</div><div class="footer__links" data-v-31e86aa7><ul class="links" data-v-31e86aa7><!--[-->`);
      ssrRenderList((_b = (_a = unref(footerLinks)) == null ? void 0 : _a.data) == null ? void 0 : _b.links, (item) => {
        _push(`<li class="link--footer" data-v-31e86aa7>`);
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: item.link
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
      ...
      _push(`<!--]--></ul></div><div class="footer__newsletter-signup" data-v-31e86aa7><span class="form-header" data-v-31e86aa7>Hear the news</span><form action="#" method="post" class="newsletter-signup" data-v-31e86aa7><input type="email" name="email" autocomplete="email" placeholder="Your email" class="input" data-v-31e86aa7><button type="submit" class="button link" data-v-31e86aa7> subscribe </button></form></div><div class="footer__social" data-v-31e86aa7><div class="social-links" data-v-31e86aa7>`);
      if (((_d = (_c = unref(footerLinks)) == null ? void 0 : _c.data) == null ? void 0 : _d.facebook_url) && unref(isFilled).link((_f = (_e = unref(footerLinks)) == null ? void 0 : _e.data) == null ? void 0 : _f.facebook_url)) {
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: (_h = (_g = unref(footerLinks)) == null ? void 0 : _g.data) == null ? void 0 : _h.facebook_url
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(unref(FacebookIcon), null, null, _parent2, _scopeId));
            } else {
              return [
                createVNode(unref(FacebookIcon))
              ];
            }
          }),
          _: 1
      ...
      if (((_j = (_i = unref(footerLinks)) == null ? void 0 : _i.data) == null ? void 0 : _j.instagram_url) && unref(isFilled).link((_l = (_k = unref(footerLinks)) == null ? void 0 : _k.data) == null ? void 0 : _l.instagram_url)) {
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: (_n = (_m = unref(footerLinks)) == null ? void 0 : _m.data) == null ? void 0 : _n.instagram_url
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(unref(InstagramIcon), null, null, _parent2, _scopeId));
            } else {
              return [
                createVNode(unref(InstagramIcon))
              ];
            }
          }),
          _: 1
      ...
      if (((_p = (_o = unref(footerLinks)) == null ? void 0 : _o.data) == null ? void 0 : _p.tiktok_url) && unref(isFilled).link((_r = (_q = unref(footerLinks)) == null ? void 0 : _q.data) == null ? void 0 : _r.tiktok_url)) {
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: (_t = (_s = unref(footerLinks)) == null ? void 0 : _s.data) == null ? void 0 : _t.tiktok_url
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(unref(TikTokIcon), null, null, _parent2, _scopeId));
            } else {
              return [
                createVNode(unref(TikTokIcon))
              ];
            }
          }),
          _: 1
      ...
      if (((_v = (_u = unref(footerLinks)) == null ? void 0 : _u.data) == null ? void 0 : _v.youtube_url) && unref(isFilled).link((_x = (_w = unref(footerLinks)) == null ? void 0 : _w.data) == null ? void 0 : _x.youtube_url)) {
        _push(ssrRenderComponent(_component_PrismicLink, {
          field: (_z = (_y = unref(footerLinks)) == null ? void 0 : _y.data) == null ? void 0 : _z.youtube_url
        }, {
          default: withCtx((_, _push2, _parent2, _scopeId) => {
            if (_push2) {
              _push2(ssrRenderComponent(unref(YouTubeIcon), null, null, _parent2, _scopeId));
            } else {
              return [
                createVNode(unref(YouTubeIcon))
              ];
            }
          }),
          _: 1
      ...
      _push(`</div></div><div class="footer__credits" data-v-31e86aa7><p data-v-31e86aa7> \xA9 ${ssrInterpolate((/* @__PURE__ */ new Date()).getFullYear())} Spread the Jelly. Site development by <a href="https://historyofsalad.com" target="_blank" data-v-31e86aa7> History of Salad. </a></p></div></footer><!--]-->`);
    };
  }
});
...
const __nuxt_component_2 = /* @__PURE__ */ _export_sfc(_sfc_main$1, [["__scopeId", "data-v-31e86aa7"]]);
const _sfc_main = /* @__PURE__ */ defineComponent({
  __name: "default",
  __ssrInlineRender: true,
  setup(__props) {
    gsap$3.registerPlugin(ScrollTrigger, ScrollSmoother);
    ...
    const title = computed(() => {
      var _a2;
      var _a;
      return (_a2 = (_a = settings.value) == null ? void 0 : _a.data.site_title) != null ? _a2 : "Spread the Jelly";
    });
    const description = computed(() => {
      var _a2;
      var _a;
      return (_a2 = (_a = settings.value) == null ? void 0 : _a.data.meta_description) != null ? _a2 : "An editorial resource & storytelling platform for every motherhood journey.";
    });
    const ogImage = computed(() => {
      var _a2;
      var _a;
      return (_a2 = prismic.asImageSrc((_a = settings.value) == null ? void 0 : _a.data.og_image)) != null ? _a2 : "/spread-the-jelly--og.png";
    });
    useSeoMeta({
      title,
      description,
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_NewsletterSignup = __nuxt_component_1;
      const _component_Footer = __nuxt_component_2;
      _push(`<div${ssrRenderAttrs(mergeProps({ class: "fade-container" }, _attrs))}>`);
      _push(ssrRenderComponent(_component_Header, null, null, _parent));
      _push(ssrRenderComponent(_component_NewsletterSignup, null, null, _parent));
      _push(`<div id="smooth-wrapper" data-scroll-container><div id="smooth-content" data-scroll-content>`);
...
//# sourceMappingURL=default-ehPvfT_0.mjs.map
